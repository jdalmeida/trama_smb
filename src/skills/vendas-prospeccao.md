...
3. **Use a busca na web** (`buscaWeb`) para verificar cada hipótese: a feira existe e tem edição próxima? A associação da cidade está ativa? O marketplace aceita esse tipo de produto? Use **somente fontes públicas** (sites oficiais, portais de eventos, notícias) e anote a URL em `ondeEncontrar` quando houver.
